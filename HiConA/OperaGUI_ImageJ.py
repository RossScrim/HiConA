...
class OperaGUI:
    ...
    def src_window(self):
        ...
        self.save_button = ttk.Button(self.directoryframe, text="...",
                                      command=lambda: self.get_directory("save_button"))
    ...
    def process_window(self):
        ...
        ttk.Label(self.root, text='2D Processing Options', font=("Segoe UI", 14)).grid(column=1, row=0, padx=70, pady=0,
                                                                                       sticky=tk.W)
        ...
        self.edfproj_check = ttk.Checkbutton(option_frame, text="Perform EDF projection on BF channel",
                                             variable=self.edfproj_state, command=self.show_imagej_frame).grid(row=3,
                                                                                                               column=0,
                                                                                                               sticky=tk.W)
        ...
        try:
            ...
            pass
        edfproj_BF_label = ttk.Label(option_frame, text="BF channel number:", width=2).grid(row=4, column=0,
                                                                                            sticky=tk.EW, padx=18)
        self.edfproj_BFch_entry = ttk.Entry(option_frame, text=self.edfproj_BFch_var, width=2, background='White').grid(
            row=4, column=0, sticky=tk.W, padx=135)
        ...
        try:
            ...
            pass
        self.stitching_entry = ttk.Entry(option_frame, text=self.stitching_var_width, width=2, background='White').grid(
            row=7, column=0, sticky=tk.W, padx=20)
        stitching_label = ttk.Label(option_frame, text="x").grid(row=7, column=0, sticky=tk.W, padx=40)
        self.stitching_entry = ttk.Entry(option_frame, text=self.stitching_var_height, width=2,
                                         background='White').grid(row=7, column=0, sticky=tk.W, padx=52)
        self.stitch_ref_var = tk.IntVar()
        try:
            self.stitch_ref_var.set(self.saved_var["stitch_ref_ch"])
        except:
            pass
        stitch_ref_label = ttk.Label(option_frame, text="Stitching ref channel:").grid(row=8, column=0, sticky=tk.W,
                                                                                       padx=18)
        self.stitch_ref_entry = ttk.Entry(option_frame, text=self.stitch_ref_var, width=2, background="White").grid(
            row=8, column=0, padx=135, sticky=tk.W)
        ...
        self.imagej_check = ttk.Checkbutton(option_frame, text="Run ImageJ Macro",
                                            variable=self.imagej_state, command=self.show_imagej_frame).grid(row=10,
                                                                                                             column=0,
                                                                                                             sticky=tk.W)
        ...
        ttk.Label(self.imagej_frame, text='ImageJ Options', font=("Segoe UI", 14)).grid(row=0, column=1, padx=0,
                                                                                        sticky=tk.NW)
        self.imagej_label = ttk.Label(self.imagej_frame, text="Fiji.app directory    ").grid(row=1, column=0, padx=0,
                                                                                             pady=10, sticky=tk.W)
        ...
        self.imagej_button = ttk.Button(self.imagej_frame, text="...",
                                        command=lambda: self.get_directory("imagej_button"), width=5)
        ...
        try:
            ...
            pass
        self.interactive_check = ttk.Checkbutton(self.macro_selection_frame, text="Interactive mode",
                                                 variable=self.interactive_state)
        self.interactive_check.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)

        self.macro_label = ttk.Label(self.macro_selection_frame, text="ImageJ macro").grid(row=1, column=0, padx=0,
                                                                                           pady=5, sticky=tk.W)
        ...
        try:
            ...
            pass
        self.macro_selected = ttk.Entry(self.macro_selection_frame, text=self.macro_entry_text, width=25,
                                        state='readonly')
        self.macro_selected.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)

        self.macro_button = ttk.Button(self.macro_selection_frame, text="...",
                                       command=lambda: self.get_directory("macro_button"), width=5)
        self.macro_button.grid(row=1, column=2, padx=0, pady=5, sticky=tk.W)

        self.arg_label = ttk.Label(self.macro_selection_frame, text="ImageJ Arguments").grid(row=2, column=0, padx=0,
                                                                                             pady=5, sticky=tk.W)
        ...
        self.arg_button = ttk.Button(self.macro_selection_frame, text="...",
                                     command=lambda: self.get_directory("arg_button"), width=5)
        self.arg_button.grid(row=2, column=2, padx=0, pady=5, sticky=tk.W)

        self.imagej_process_label = ttk.Label(self.macro_selection_frame, text="Apply macro").grid(row=3, column=0,
                                                                                                   sticky=tk.W)
        ...
        try:
            ...
            pass
        self.imagej_combobox = ttk.Combobox(self.macro_selection_frame, textvariable=self.imagej_order_text,
                                            state='readonly', values=["to stack", "after MIP/EDF", "after stitching"])
        self.imagej_combobox.grid(row=3, column=1, sticky=tk.W, padx=5, pady=5)

        imagej_information_button = ttk.Button(self.macro_selection_frame, text="Macro/args files guidelines",
                                               command=self.display_imagej_info)
    ...
    def display_imagej_info(self):
        messagebox.showinfo(title="Guidelines for ImageJ macro and arguments files",
                            message="The macro should be an .ijm and the arguments a .json file.\n\n"
    ...
    def src_processing(self):
        ...
        for measurement in [f for f in os.listdir(self.src_dir) if
                            (os.path.isdir(os.path.join(self.src_dir, f)) and f != "_configdata")]:
    ...
    def proc_confirm(self):
        ...
        # No 3D options added yet - add here when needed
            ...
            messagebox.showinfo(title="Missing Information",
                                message="Please choose the location for the Fiji.app directory")
        elif (self.imagej_state.get() == 1) and (imagej_macro == "" or imagej_args == ""):
            messagebox.showinfo(title="Missing Information",
                                message="Please choose the location for the ImageJ macro and arguments files")
        elif self.imagej_state.get() == 1 and imagej_proc_order == "":
            messagebox.showinfo(title="Missing Information",
                                message="Please choose when the macro should be applied in the dropdown menu.")
        elif self.stitching_state.get() == 1 and (
                not isinstance(stitch_width, int) or stitch_width == 0 or not isinstance(stitch_height,
                                                                                         int) or stitch_height == 0):
            messagebox.showinfo(title="Missing Information",
                                message="Please provide the dimensions for the stitched image")
        elif self.stitching_state.get() == 1 and (not isinstance(stitch_ref_ch, int) or stitch_ref_ch == 0):
            messagebox.showinfo(title="Missing Information",
                                message="Please provide the stitching reference channel number")
        else:
            # Save imagej config file using json
            if self.imagej_state.get() == 1:
                imagej_dict = {"fiji_loc": imagej_loc, "macro_file": imagej_macro, "args_file": imagej_args,
                               "interactive": interactive_mode, "proc_order": imagej_proc_order}
            ...
            self.measure_to_process = [self.measurement_dict[list(self.measurement_dict.keys())[i]] for i in
                                       range(len(self.measurement_dict)) if self.measure_var_list[i].get() == 1]
            self.processes_to_run = {k: v.get() for k, v in self.processing_options.items()}
            self.root.destroy()

            for cur_measurement in self.measure_to_process:
                cur_plate_name = list(self.measurement_dict.keys())[
                    list(self.measurement_dict.values()).index(cur_measurement)]
...
class HiConAProcessing:
    def __init__(self, files, processes_to_run, save_dir, BFchannel, stitch_width, stitch_height, stitch_ref_ch,
                 imagej_loc, imagej_proc_order):
        self.files = files
        self.save_dir = save_dir
        self.processes_to_run = processes_to_run  # Dict with keys = process function name

        self.BFchannel = BFchannel
    ...
    def _load_config_file(self):
        return OperaExperimentConfigReader(
    ...
    def _load_images(self, filepaths):
        first_image = tifffile.imread(filepaths[0])
        self.ydim, self.xdim = first_image.shape()
        im_arr = [first_image]

    def _save_images(self, full_file_path, images):
        tifffile.imwrite(full_file_path,
                images,
                imagej=True, metadata={'axes': f'{self.axes}'})

    def _get_filepaths(self, cur_well, image_pattern):
        return
    ...
    def _build_imagename_pattern(self, FOV, timepoint=None):
        pattern = fr"r\d+c\d+f0?{FOV}p\d+-ch\d+t"
        if timepoint is not None:
            pattern += fr"0?{timepoint}"
        pattern += r".tiff?"
        return pattern

    def _process_fov(self, images):
    ...
    def _process_well(self, cur_well):
        ...
        if self.timepoints > 1:
            self._run_timelapse_pipeline(cur_well)
        else:
            self._run_single_timepoint_pipeline(cur_well)

    def _run_timelapse_pipeline(self, cur_well):
        for cur_FOV in range(1, self.FOVs + 1):
            for cur_time in range(1, self.timepoints + 1):
                image_pattern = self._build_imagename_pattern(cur_FOV, cur_time)
                full_
                try:
                    images = self.files.get_opera_phenix_images_from_FOV(cur_well, image_pattern)
                    self._process_fov(images)
                except ValueError as e:
                    print("Error processing well " + cur_well + " field " + str(cur_FOV) + " timepoint " + str(
                        cur_time) + " with ValueError.")
                    continue

    def _run_single_timepoint_pipeline(self, cur_well):
        for cur_FOV in range(1, self.FOVs + 1):
            image_pattern = self._build_imagename_pattern(cur_FOV)
            try:
                images = self.files.get_opera_phenix_images_from_FOV(cur_well, image_pattern)
                self._process_fov(images)
            except ValueError as e:
...
>>>>>>> origin/refactor-imageProcessor
    def run(self):
        for cur_well in self.files.well_names:
            cur_save = os.path.join(self.save_dir, cur_well)
            self.files.create_dir(cur_save)
            ...
            if not self.is_data_timelapse:
                for cur_FOV in range(1, self.FOVs + 1):
                    ...
                    try:
                        if np.ndim(processor.get_image()) == 4:
                            tifffile.imwrite(
                                cur_save + "/" + cur_well + "f" + self.FOV_rename_order[cur_FOV - 1] + ".tif",
                            ...
                            if self.processes_to_run["separate_ch"] == 1:
                                for ch in range(self.channels):
                                    ...
                                    tifffile.imwrite(ch_save_path + "/" + cur_well + "f" + self.FOV_rename_order[
                                        cur_FOV - 1] + ".tif",
                                                     processor.get_image()[:, ch, :, :], imagej=True,
                                                     metadata={'axes': 'ZYX'})
                        else:
                            tifffile.imwrite(
                                cur_save + "/" + cur_well + "f" + self.FOV_rename_order[cur_FOV - 1] + ".tif",
                ...
                if self.processes_to_run["stitching"] == 1 and len(
                        [f for f in os.listdir(cur_save) if f.endswith(".tif")]) == len(self.FOV_rename_order):
                    try:
                        StitchProcessing(cur_save, self.stitch_ref_ch, self.imagej_loc, self.stitch_height,
                                         self.stitch_width)
                ...
                if self.imagej_proc_order == "after stitching":
                    ...
                    try:
                        ...
                        post_stitched_image_name = [os.path.join(stitch_loc, image_files[i]) for i in
